 ...
 * @LastEditTime: 2025-09-27 19:52:28
...
function get_oauth_config($type = 'qq')
{
    ...
    // 如果是google登录,则使用子主题的配置
    if ($type === 'google') {
        return wp_parse_args((array) _mrhe('oauth_google_option'), $defaults);
    }
    
    // 如果是mixauthqq登录,则使用子主题的配置
    if ($type === 'mixauthqq') {
        return wp_parse_args((array) _mrhe('oauth_mixauthqq_option'), $defaults);
    }

    return wp_parse_args((array) _pz('oauth_' . $type . '_option'), $defaults);
}

function zib_oauth_google_login_url_filter($url, $type) {
    if ($type === 'google' && _mrhe('oauth_google_s')) {
        $url = home_url('oauth/google');
    }
    return $url;
}
add_filter('zib_oauth_login_url', 'zib_oauth_google_login_url_filter', 10, 2);

function zib_oauth_mixauthqq_login_url_filter($url, $type) {
    if ($type === 'mixauthqq' && _mrhe('oauth_mixauthqq_s')) {
        $url = home_url('oauth/mixauthqq');
    }
    return $url;
}
add_filter('zib_oauth_login_url', 'zib_oauth_mixauthqq_login_url_filter', 10, 2);

// AJAX处理函数：代理MixAuth状态检查请求
function mixauthqq_check_status_ajax() {
    // 设置JSON响应头
    ...
    if (empty($server_url) || empty($qr_id)) {
        ...
        echo json_encode(['error' => '参数缺失：server_url=' . $server_url . ', qr_id=' . $qr_id]);
        wp_die();
    }
    ...
    if (is_wp_error($response)) {
        ...
        echo json_encode(['error' => 'MixAuth服务请求失败: ' . $error_message]);
        wp_die();
    }
...
add_action('wp_ajax_mixauthqq_check_status', 'mixauthqq_check_status_ajax');
add_action('wp_ajax_nopriv_mixauthqq_check_status', 'mixauthqq_check_status_ajax');

function zib_output_users_columns_filter($var, $column_name, $user_id) {
    if ($column_name === 'oauth') {
        $user = get_userdata($user_id);
        $args = array(
            array('name' => 'QQ', 'type' => 'qq'),
            array('name' => '微信', 'type' => 'weixin'),
            array('name' => '微信', 'type' => 'weixingzh'),
            array('name' => '微博', 'type' => 'weibo'),
            array('name' => 'GitHub', 'type' => 'github'),
            array('name' => '码云', 'type' => 'gitee'),
            array('name' => '百度', 'type' => 'baidu'),
            array('name' => '支付宝', 'type' => 'alipay'),
            array('name' => 'Google', 'type' => 'google'),
            array('name' => 'QQ', 'type' => 'mixauthqq'),
        );
...
add_filter('manage_users_custom_column', 'zib_output_users_columns_filter', 11, 3); 
...
function zib_oauth_page_template()
{
    ...
    if ($oauth) {
        ...
        $allow_list = array(
            ...
            'baidu', 'baiduagent',
            'qq', 'qqagent',
            'weixin', 'weixinagent', 'weixingzh',
            'weibo', 'weiboagent',
            'github', 'githubagent',
            'google', 'mixauthqq'
        );

        if (in_array($oauth, $allow_list)) {
            global $wp_query;
            $wp_query->is_home = false;
            $wp_query->is_page = false;

            // 构造文件路径（子主题优先，父主题兜底）
            $template_file = '/oauth/' . $oauth . ($oauth_callback ? '/callback.php' : '/login.php');
