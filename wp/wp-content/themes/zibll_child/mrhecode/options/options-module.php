...
class MrheAut
{
    ...
    {
        if (class_exists('MrheAuthClient')) {
            return MrheAuthClient::getInstance()->is_theme_authorized();
        }
        return false;
    }
...
//未授权时的拦截逻辑
if (!MrheAut::is_aut() && !MrheAut::is_local()) {
    //后台保存拦截
    add_action("csf_mrhe_options_save_before", function() {
    ...
    add_action('admin_footer', function() {
        ...
        jQuery(document).ready(function($) {
            $(document).on("click", ".csf-buttons", function() {
                var r = confirm("mrhe主题还未授权，暂时无法使用此功能。开始授权验证？");
                if (r == true) {
                    self.location.href="/wp-admin/admin.php?page=mrhe_options#tab=主题授权";
                }
                return false;
            });
        });
...
function mrhe_admin_curl_aut()
{
    ...
    if (empty($auth_code)) {
        //无授权码 → 自动授权
        $result = MrheAuthClient::getInstance()->auto_verify();
    } else {
        //有授权码 → 手动授权
        $result = MrheAuthClient::getInstance()->verify_auth_with_code($auth_code);
    }
...
add_action('wp_ajax_mrhe_admin_curl_aut', 'mrhe_admin_curl_aut');

//AJAX处理：撤销授权
function mrhe_curl_delete_authorization()
{
    if (!class_exists('MrheAuthClient')) {
        echo json_encode(array('error' => 1, 'msg' => '授权模块未加载'));
        exit;
    ...
    $result = MrheAuthClient::getInstance()->revoke_auth();
...
add_action('admin_notices', 'mrhe_auto_aut', 99);

//CSF模块类
class CFS_Module_mrhe
{
    /**
     * 生成授权界面HTML
    ...
    {
        // 添加调试信息
        error_log('CFS_Module_mrhe::aut() 被调用');
        error_log('MrheAut 类存在: ' . (class_exists('MrheAut') ? 'YES' : 'NO'));
        error_log('MrheAuthClient 类存在: ' . (class_exists('MrheAuthClient') ? 'YES' : 'NO'));
        
        $is_aut = MrheAut::is_aut();
        $is_local = MrheAut::is_local();
        
        error_log('is_aut: ' . ($is_aut ? 'TRUE' : 'FALSE'));
        ...
        if ($is_aut) {
            ...
            <input type="hidden" ajax-name="action" value="mrhe_curl_delete_authorization">
            ...
            <input type="hidden" ajax-name="action" value="mrhe_admin_curl_aut">
            </div>
            <a id="authorization_submit" class="but c-blue ajax-submit curl-aut-submit" data-depend-id="mrhe_submit_aut">一键授权</a>
            <div class="ajax-notice"></div>
            </div>';
        }
    ...
    public static function backupmrhe()
    {
        $csf            = array();
        $prefix         = 'mrhe_options';
        ...
        if ($options) {
            ...
            foreach ($options as $key => $val) {
                ...
                $del      = '<a href="javascript:;" ajax-url="' . add_query_arg('action', 'mrhe_options_backup_delete', $ajax_url) . '" data-confirm="确认要删除此备份[' . $key . ']？删除后不可恢复！" class="but c-yellow ajax-get ml10">删除</a>';
                $restore  = '<a href="javascript:;" ajax-url="' . add_query_arg('action', 'mrhe_options_backup_restore', $ajax_url) . '" data-confirm="确认将主题设置恢复到此备份吗？[' . $key . ']？" class="but c-blue ajax-get ml10">恢复</a>';
            ...
            if ($count > 3) {
                $delete_but = '<a href="javascript:;" ajax-url="' . add_query_arg(array('action' => 'mrhe_options_backup_delete_surplus', 'key' => 'all'), $admin_ajax_url) . '" data-confirm="确认要删除多余的备份数据吗？删除后不可恢复！" class="but jb-red ajax-get">删除备份 保留最新三份</a>';
            }
        }
        $csf[] = array(
            'type'    => 'submessage',
            ...
            <a <a href="javascript:;" ajax-url="' . add_query_arg('action', 'mrhe_options_backup', $admin_ajax_url) . '" class="but jb-blue ajax-get">备份当前配置</a>
            ' . $delete_but . '
        ...
        $csf[] = array(
            ...
            <input type="hidden" ajax-name="action" value="mrhe_options_import">
...
// 延迟加载授权模块的函数
function mrhe_get_authorization_module_content() {
    // 确保在 WordPress 完全加载后调用
    if (class_exists('CFS_Module_mrhe') && method_exists('CFS_Module_mrhe', 'aut')) {
        $auth_module = CFS_Module_mrhe::aut();
        if (is_array($auth_module) && isset($auth_module['content'])) {
            return $auth_module['content'];
        }
    }
    
    // 如果类还没有加载，返回默认的未授权状态
    return '<div id="authorization_form">
    <p>系统正在加载授权模块，请稍候...</p>
    </div>';
}
