...
function zib_main_register_sidebar()
{
    if (function_exists('register_sidebar')) {
        ...
        $sidebars[] = array(
            'name'        => '产品页面',
            'id'          => 'product_sidebar',
            'description' => '显示在产品页面（通用后备侧边栏）',
        );
        
        // 何先生新增会员开通页面
        $sidebars[] = array(
            'name'        => '会员开通页面',
            'id'          => 'product_vip_sidebar',
            'description' => '会员开通页面,需要后台新建会员开通模版页面',
        );
...
// 产品页面小工具（仿照 Zibll 官方风格）
function mrhe_product_register_sidebar()
{
    $cache = wp_cache_get('mrhe_product_pages', 'zib_cache_group', true);
    ...
    if (false === $cache) {
        ...
        $cache = get_posts(array(
            ...
            'meta_value'     => 'pages/mrhe_pay.php',
        ));
        
        wp_cache_set('mrhe_product_pages', $cache, 'zib_cache_group');
    }
    ...
    foreach ($cache as $page_id) {
        $page_title = get_the_title($page_id);
        $name = zib_str_cut($page_title, 0, 10, '...');
        
        register_sidebar(array(
            'name'          => '[产品页面:' . $name . ']-侧边栏',
            'id'            => 'product_sidebar_' . $page_id,
            'description'   => '显示在产品页面"' . $page_title . '"的侧边栏',
            'before_widget' => '<div class="zib-widget %2$s">',
            'after_widget'  => '</div>',
            'before_title'  => '<h3>',
            'after_title'   => '</h3>',
...
add_action('widgets_init', 'mrhe_product_register_sidebar');

// 保存页面时刷新缓存（仿照 Zibll 官方风格）
function mrhe_product_pages_update_cache()
{
    wp_cache_delete('mrhe_product_pages', 'zib_cache_group');
}
add_action('save_post_page', 'mrhe_product_pages_update_cache');
