 ...
 * @LastEditTime: 2025-09-28 21:00:48
...
function zib_get_comments_user_name($comment)
{
    ...
    //何先生新增代码需结合function里mrhee_comment_author_link()函数
    elseif(empty($user_id) && !empty($comment->comment_author_url)){
...
function zib_get_comment_header($comment)
{
    ...
    $author_avatar = zib_get_avatar_box($user_id ? $user_id : $comment->comment_author_email, 'avatar-img comt-avatar');
...
function zib_get_comment_footer($comment, $depth = 0)
{
    ...
    if (_pz('comment_city_s')) {
        if($comment->user_id && user_can($comment->user_id, 'manage_options') && _mrhe('admin_ip_address_swich') && _mrhe('admin_ip_address')) {
            // 如果是管理员且开启了自定义显示，并且设置了自定义文字，则显示自定义文字
            $addr_html = '<span class="badg badg-sm">' . _mrhe('admin_ip_address') . '</span>';
        } else {
            // 如果不是管理员或未开启自定义显示或未设置自定义文字，显示正常地理位置
...
function zib_widget_comments($limit, $outpost, $outer)
{
    ...
    foreach ($comments as $comment) {
        ...
        $avatar = zib_get_avatar_box($user_id ? $user_id : $comment->comment_author_email);
...
function zib_get_respond_mobile($input_s = '#respond', $placeholder = '', $class = '', $is_lognin = false)
{
    ...
    $user_id = $user_id ? $user_id : $current_commenter['comment_author_email'];
