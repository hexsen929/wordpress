...
function zib_single_after_box()
{
    ...
    if (_mrhe('adsense_on_related')){//何先生新增代码
        echo '<div class="theme-box relates relates-thumb">'._mrhe('adsense_js_to_related').'</div>';
    }
...
function zib_single_content_footer($post)
{
    ...
    if (_pz('post_copyright_s')) {
        $from = '';
        //何先生新增代码
        mrhe_moloader('mo_get_post_from', false);
        if( mo_get_post_from() ){ 
            $from = '<span class="item">'.mo_get_post_from().'</span><br />';
        };
        echo '<div class="em09 muted-3-color"><div><span>©</span> 版权声明</div>'.$from.'<div class="posts-copyright">' . _pz('post_copyright') . '</div></div>';
    }
