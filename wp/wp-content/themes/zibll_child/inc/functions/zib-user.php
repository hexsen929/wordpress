...
function zib_get_sign_captch($captcha_type = 'email', $action = 'signup_captcha')
{
    ...
    //$input .= wp_nonce_field($action, '_wpnonce', false, false); //安全效验
    $input .= '<input type="hidden" name="_wpnonce" class="sign_nonce" value="" data-action="' . esc_attr($action) . '">';
    $input .= '</div>';
    return $input;
}

// 提供 AJAX 获取 nonce
add_action('wp_ajax_get_sign_nonce', 'get_sign_nonce');
add_action('wp_ajax_nopriv_get_sign_nonce', 'get_sign_nonce');

function get_sign_nonce() {
    if (empty($_GET['action_name'])) {
        wp_send_json_error('缺少 action_name');
    }
    $action = sanitize_text_field($_GET['action_name']);
...
{
    //这里加一个判断，如果$id是纯数字开头的邮箱，则将评论区未登陆用户的头像url改写
    //避免暴露qq号
...
function zib_get_avatar_box($user_id, $class = 'avatar-img', $link = true, $vip = true)
{
    ...
    if (is_string($user_id) && (strpos($user_id, '@') !== false)) {
        $user_id = 0;
    } //何先生新增
...
function zib_get_user_avatar_url($user_id)
{
        ...
        if (is_numeric($user_id)) {
            ...
            } elseif (!empty($user_id->comment_author_email)) {
                $email = $user_id->comment_author_email;
            }
        } else {
            $email = $user_id;
        }
        ...
        if ($email) {
            $avatar = mrhe_get_gravatar_url($email, $args['size']);
        } else {
            // 3. 如果既没有自定义头像也没有邮箱，使用默认头像
            $avatar = zib_default_avatar();
        }
    return zib_get_user_meta($user_id, 'custom_avatar', true) ?: $avatar;
}
