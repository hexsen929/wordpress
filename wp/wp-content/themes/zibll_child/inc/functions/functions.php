 ...
 * @LastEditTime: 2025-09-19 11:22:30
...
function zib_system_notice()
{
    ...
    zib_modal($args);
    $expires = round(_pz('system_notice_expires', 24), 3); // 保持小时为单位
    $script  = '<script type="text/javascript">';
    $script .= 'window.onload = function(){
        var lastShown = localStorage.getItem("showed_system_notice");
        var now = Date.now();
        var hours = ' . $expires . ';
        var expireTime = hours * 60 * 60 * 1000; // 转换小时为毫秒
        
        // 如果设置为0，则清除存储并始终显示
        if(' . $expires . ' <= 0) {
            localStorage.removeItem("showed_system_notice");
            lastShown = null;
        }
        
        // 如果从未显示过或者已经超过设定时间
        if(!lastShown || (now - parseInt(lastShown)) > expireTime){
            setTimeout(function () {
                $("#modal-system-notice").modal("show");
                if(' . $expires . ' > 0) { // 只有在不为0时才存储时间戳
                    localStorage.setItem("showed_system_notice", now);
                }
            }, 500);
        }
    };';
...
remove_action('wp_footer', 'zib_system_notice');
...
function zib_social_login($echo = true)
{
    ...
    if (_pz('social') && function_exists('xh_social_loginbar')) {
        ...
        foreach ($args as $arg) {
            ...
            // 对于Google和MixAuth QQ登录,优先检查插件配置
            if ($type === 'google') {
                // 先检查插件是否启用
                $is_enabled = apply_filters('zib_oauth_' . $type . '_enabled', _mrhe('oauth_google_s'), $type);
            } elseif ($type === 'mixauthqq') {
                $is_enabled = apply_filters('zib_oauth_' . $type . '_enabled', _mrhe('oauth_mixauthqq_s'), $type);
            } else {
                $is_enabled = _pz('oauth_' . $type . '_s');
            }
...
function zib_get_social_type_data()
{
    ...
    // 允许插件和子主题扩展社交登录类型
    $args = apply_filters('zib_social_type_data', $args);
...
/**
 * 从多级JSON数据中获取值
 * @param array $array JSON解析后的数组
 * @param string $path 使用点号分隔的路径，如 data.location.country
...
function zib_get_geographical_position_by_ip($ip)
{
    ...
    // 检查是否启用了子主题的IP查询功能
    if (_mrhe('ip_location_s')) {
        $api = _mrhe('ip_location_api');
        $polling_enabled = _mrhe('ip_location_polling');
        $custom_apis = _mrhe('ip_location_custom_apis');
        $selected_custom_api = _mrhe('selected_custom_api'); // 新增：选择的自定义API的ID
        ...
        if (!$polling_enabled) {
            // 内置API调用
            if ($api === 'ip-api') {
                return $query_ip_api($ip);
            }
                
            if ($api === 'ipapi') {
                return $query_ipapi($ip);
            }

            // 自定义API调用
            if ($api === 'custom' && !empty($custom_apis) && is_array($custom_apis)) {
                // 查找选中的自定义API
                foreach ($custom_apis as $api_config) {
                    if (!empty($api_config['api_status']) && $api_config['id'] === $selected_custom_api) {
                        if (!empty($api_config['api_url'])) {
                            $api_url = str_replace('{ip}', $ip, $api_config['api_url']);
                            $response = wp_remote_get($api_url, array('timeout' => 3));
                            
                            if (!is_wp_error($response) && wp_remote_retrieve_response_code($response) === 200) {
                                $result = json_decode(wp_remote_retrieve_body($response), true);
                                if ($result) {
                                    $nation = zib_get_array_value_by_path($result, $api_config['nation_key']);
                                    $province = zib_get_array_value_by_path($result, $api_config['province_key']);
                                    $city = zib_get_array_value_by_path($result, $api_config['city_key']);
                                    
                                    if (!empty($nation) || !empty($province) || !empty($city)) {
                                        return array(
                                            'nation' => $nation ?: '',
                                            'province' => $province ?: '',
                                            'city' => $city ?: ''
        ...
        // 轮询模式处理
        if ($polling_enabled && $api === 'custom') {
            $polling_strategy = _mrhe('ip_location_polling_strategy');
            
            // 过滤出启用状态的API
            $active_apis = array();
            if (!empty($custom_apis) && is_array($custom_apis)) {
                foreach ($custom_apis as $api_config) {
                    if (!empty($api_config['api_status'])) {
                        $active_apis[] = $api_config;
                    }
                }
            }
            ...
            $built_in_apis[] = array(
                'type' => 'ip-api',
                'query_func' => $query_ip_api
            );
            $built_in_apis[] = array(
                'type' => 'ipapi',
                'query_func' => $query_ipapi
            );

            // 合并所有可用的API
            $all_apis = array_merge($built_in_apis, $active_apis);

            if (!empty($all_apis)) {
                if ($polling_strategy === 'sequential') {
                    // 顺序轮询
                    $last_api_index = get_transient('mrhe_last_api_index');
                    $current_index = ($last_api_index === false) ? 0 : ($last_api_index + 1) % count($all_apis);
                    set_transient('mrhe_last_api_index', $current_index, DAY_IN_SECONDS);
                    
                    // 尝试所有API直到成功
                    for ($i = 0; $i < count($all_apis); $i++) {
                        $index = ($current_index + $i) % count($all_apis);
                        $api_to_use = $all_apis[$index];
                        
                        if (isset($api_to_use['type'])) {
                            // 内置API
                            $result = $api_to_use['query_func']($ip);
                            if ($result) {
                                return $result;
                            }
                        } else {
                            // 自定义API
                            if (!empty($api_to_use['api_url'])) {
                                $api_url = str_replace('{ip}', $ip, $api_to_use['api_url']);
                                $response = wp_remote_get($api_url, array('timeout' => 3));
                                
                                if (!is_wp_error($response) && wp_remote_retrieve_response_code($response) === 200) {
                                    $result = json_decode(wp_remote_retrieve_body($response), true);
                                    if ($result) {
                                        $nation = zib_get_array_value_by_path($result, $api_to_use['nation_key']);
                                        $province = zib_get_array_value_by_path($result, $api_to_use['province_key']);
                                        $city = zib_get_array_value_by_path($result, $api_to_use['city_key']);
                    ...
                    // 随机轮询
                    $apis_to_try = $all_apis;
                    shuffle($apis_to_try); // 随机打乱API顺序
                    
                    foreach ($apis_to_try as $api_to_use) {
                        if (isset($api_to_use['type'])) {
                            // 内置API
                            $result = $api_to_use['query_func']($ip);
                            if ($result) {
            ...
            // 如果所有API都失败，返回false让父主题继续处理
            return false;
        }
    }

    // 如果子主题IP查询未启用，使用父主题的逻辑
    $sdk = _pz('ip_addr_sdk');
